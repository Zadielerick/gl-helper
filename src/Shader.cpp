...
Shader::Shader(GLenum shaderType, std::string source) : shaderType(shaderType), source(std::move(source)) { }

/* Vertex Shader Constructors */
VertexShader::VertexShader() : Shader(GL_VERTEX_SHADER) { }
VertexShader::VertexShader(std::string source) : Shader(GL_VERTEX_SHADER, source) { }

/* Fragment Shader Constructors */
FragmentShader::FragmentShader() : Shader(GL_FRAGMENT_SHADER) { }
FragmentShader::FragmentShader(std::string source) : Shader(GL_FRAGMENT_SHADER, source) { }

/* Shader Program Constructor */
ShaderProgram::ShaderProgram(std::string vsSource, std::string fsSource) : vs(vsSource), fs(fsSource){ }

void Shader::compileShader()
{
	if (shaderType != GL_FRAGMENT_SHADER && shaderType != GL_VERTEX_SHADER)
	{
		std::cout << " Shader: Shader Type not set to GL_FRAGMENT_SHADER or GL_VERTEX_SHADER";
		std::cout << std::endl;
		return;
	}
		...
		if (!compiled)
		{
			...
			if (infoLen)
			{
				...
				if (buf)
				{
					...
					std::cout << "GL: Could not compile shader " << shaderType << ":" << std::endl;
...
void ShaderProgram::compileShaderProgram()
{
	...
	if (program)
	{
		...
		//TODO: TestGLError("glAttachShader");
